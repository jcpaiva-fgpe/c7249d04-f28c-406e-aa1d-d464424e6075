...
while True:
    ...
    Witamy w banku!
    ===============
    1. Wyświetlenie listy wszystkich kont (pełne dane)
    2. Założenie nowego konta (z podaniem danych właściciela)
    3. Zmiana danych właściciela aktywnego konta
    4. Zablokowanie aktywnego konta
    5. Zarejestrowanie wpłaty gotówkowej na konto
    6. Zarejestrowanie wypłaty gotówkowej z konta
    7. Wykonanie przelewu między dwoma kontami
    8. Naliczenie odsetek na wszystkich aktywnych kontach
    9. Wyjście z programu
    """)
    opcja = input("Wybierz 1-9:")
    if opcja == '9': break
    elif opcja == '8': Konto.odsetki()
    elif opcja == '7': Konto.przelew()
    elif opcja == '6': Konto.wyplata()
    elif opcja == '5': Konto.wplata()
    elif opcja == '4': Konto.zamknij_konto()
    elif opcja == '3': k = Konto.dostep_do_konta("do zmiany danych: "); k.wlasciciel.podaj_dane_wlas()
    elif opcja == '2': print(Konto())
    elif opcja == '1':
        for k in Konto.zalozone_konta:
            print(k)
    else: print("Opcja nieznana!")
